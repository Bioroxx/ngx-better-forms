...
export type FormControlValuesCondition = AbstractFormControlCondition &
  InternalValuesCondition<unknown>;
export type FormControlRangeCondition = AbstractFormControlCondition &
  InternalRangeCondition<number>;
export type FormControlCondition =
  | FormControlValuesCondition
  | FormControlRangeCondition;

function isAbstractFormControlCondition(
...
export function evaluateFormControlCondition(
  control: AbstractControl,
  condition: FormControlCondition
): boolean {
...
export function evaluateConditions(
  ...
  const conditionResults = conditions.map((c) =>
    evaluateFormControlCondition(control, c)
  );
